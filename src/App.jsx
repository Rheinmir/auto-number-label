...
const App = () => {
  ...
  const [scale, setScale] = useState(1); // Auto-scale factor
  ...
  useEffect(() => {
    const calculateScale = () => {
      ...
      const sidebarWidth = 320; // 80 * 4 = w-80
      const padding = 64; // p-8 = 32px * 2
      const availableWidth = window.innerWidth - sidebarWidth - padding;
      const availableHeight = window.innerHeight - padding - 80; // minus info bar

      // A4 dimensions in mm converted to pixels (assuming 96 DPI)
      const a4Width = 210 * 3.7795; // ~793px
      const a4Height = 297 * 3.7795; // ~1122px
  ...
  return (
    <div className="min-h-screen bg-[#0f172a] text-slate-100 font-sans pb-10 print:bg-white print:p-0 select-none">
      ...
      <div
        ref={previewAreaRef}
        className="ml-80 p-8 flex flex-col items-center print:m-0 print:p-0 min-h-screen"
      >
        <div
          className="relative print:hidden group"
          style={{
